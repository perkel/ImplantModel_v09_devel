...
def cylinder3d_makeprofile(field_table, fieldParams, simParams):
    # Some preliminaries
    eidx = simParams['channel']['number']  # channel index into vector fields of 'simParams'; must be an integer
    # Test for fatal errors    
    if simParams['cochlea']['radius'][eidx] != fieldParams['cylRadius']:
    ...
    zspace = fieldParams['zEval']

    # Set up scaling and translation instructions for stimulation, which depends on electrode configuration #
    if simParams['channel']['config'] == 'pTP':
        sigma = simParams['channel']['sigma']
        alpha = simParams['channel']['alpha']
            
        v_interp = []
        if sigma > 0.0:  # tripolar
            v_interp.append(cylinder_interpolate(field_table, fieldParams, simParams, eidx))
            v_interp.append(cylinder_interpolate(field_table, fieldParams, simParams, eidx))
            v_interp.append(cylinder_interpolate(field_table, fieldParams, simParams, eidx))
            vloci = simParams['electrodes']['zpos'][eidx + np.array([-1, 0, 1])]
        else:  # skip some processing steps for monopolar condition
            v_interp.append(np.zeros(field_table.shape[1]))
            v_interp.append(cylinder_interpolate(field_table, fieldParams, simParams, eidx))
            v_interp.append(np.zeros(field_table.shape[1]))
            vloci = simParams['electrodes']['zpos'][eidx + np.zeros(3, dtype=int)]
                         
        vscaling = [-sigma*(1-alpha), 1.0, -sigma*alpha]

    else:
        raise('Configuration ', simParams['channel']['config'], ' is not recognized.')
    
    zgrid = simParams['grid']['z']
