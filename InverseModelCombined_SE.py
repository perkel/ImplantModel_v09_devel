...
ifPlotGuessContours = False
use_fwd_model = False  # Whether to use output from the forward model or, alternatively, from subject
...
#         par.add('max_adj', expr=max_diff_adjacent(x, par), max=0.5)
        ...
        # Summary data  ## TODO save monopolar and tripolar errors separately.
        ...
        with open(save_file_name, mode='w') as data_file:
            ...
            for row in range(0, NELEC):
                ...
                if use_fwd_model:
                    ...
                    t10 = ct_vals[row]
                    data_writer.writerow([t1, t2, t3, t4, t5, t6, t7, t8, t9, t10])
        data_file.close()
        ...
        if use_fwd_model:
            np.save(save_file_name, [sigmaVals, rposvals, survvals, thrsim, thrtargs, initvec, [fitrposvals, fitsurvvals],
                                 rposerrs, rpos_err_metric, survivalerrs])
        else:
            np.save(save_file_name, [sigmaVals, rposvals, survvals, thrsim, thrtargs, initvec, [fitrposvals, fitsurvvals],
                                 rposerrs, rpos_err_metric, survivalerrs, ct_vals])
    ...
    with open(summary_file_name, mode='w') as data_file:
        data_writer = csv.writer(data_file, delimiter=',', quotechar='"')
        header = ['Scenario/subject', 'Mean MP Threshold Error', 'Mean TP Threshold Error', 'Position Error', 'Dist correlation', 'Dist corr p']
        data_writer.writerow(header)
    ...
    np.save(summary_file_name, [scenarios, rpos_summary])
